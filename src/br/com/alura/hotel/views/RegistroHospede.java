...
public class RegistroHospede extends JFrame {
	...
	public RegistroHospede(Reserva reserva) {
		...
		txtDataNasc = new JDateChooser();
		txtDataNasc.setBounds(560, 278, 285, 36);
		txtDataNasc.getCalendarButton()
				.setIcon(new ImageIcon(RegistroHospede.class.getResource("/imagens/icon-reservas.png")));
		txtDataNasc.getCalendarButton().setBackground(SystemColor.textHighlight);
		txtDataNasc.setDateFormatString("yyyy-MM-dd");
		...
		txtNacionalidade.setModel(new DefaultComboBoxModel<String>(new String[] { "alemão", "andorrano", "angolano",
		...
		lblDataNasc.setBounds(560, 256, 255, 14);
		lblDataNasc.setForeground(SystemColor.textInactiveText);
		lblDataNasc.setFont(new Font("Roboto Black", Font.PLAIN, 18));
		...
		txtNomeReserva.setFont(new Font("Roboto", Font.PLAIN, 16));
		txtNomeReserva.setBounds(560, 495, 285, 33);
		txtNomeReserva.setColumns(10);
		txtNomeReserva.setBackground(Color.WHITE);
		txtNomeReserva.setBorder(javax.swing.BorderFactory.createEmptyBorder());
		txtNomeReserva.setText(String.valueOf(reserva.getId()));
		contentPane.add(txtNomeReserva);
		...
		panel.add(logo);
		logo.setIcon(new ImageIcon(RegistroHospede.class.getResource("/imagens/logo-hotel-100px.png")));
	}

	private void salvar() {
		LocalDate dataNasc = txtDataNasc.getDate().toInstant().atZone(ZoneId.systemDefault()).toLocalDate();

		Hospede hospede = new Hospede(
				txtNome.getText(), txtSobrenome.getText(), dataNasc,
				(String) txtNacionalidade.getSelectedItem(), txtTelefone.getText(),
				Integer.parseInt(txtNomeReserva.getText()));

		this.hospedeController.salvar(hospede);
		JOptionPane.showMessageDialog(null,
				"Cadastro bem-sucedido. O número da sua reserva é: " + txtNomeReserva.getText());
	}
